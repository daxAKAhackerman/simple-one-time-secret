<template>
  ...
  <b-row>
    <b-col cols="6">
      <b-input-group>
        <b-form-input v-model="expirationDate" type="text" placeholder="YYYY-MM-DD" autocomplete="off"
          required></b-form-input>
        <b-input-group-append>
          <b-form-datepicker v-model="expirationDate" button-only right locale="en-US"></b-form-datepicker>
        </b-input-group-append>
      </b-input-group>
    </b-col>
    <b-col cols="6">
      <b-input-group>
        <b-form-input v-model="expirationTime" type="text" placeholder="HH:mm:ss" required></b-form-input>
        <b-input-group-append>
          <b-form-timepicker v-model="expirationTime" button-only right locale="en" show-seconds
            :hour12="false"></b-form-timepicker>
        </b-input-group-append>
      </b-input-group>
    </b-col>
  </b-row>
  ...
  <ol>
    <li>
      Using JavaScript, a random encryption key and IV are generated and used
      to encrypt the secret.
    </li>
    <li>
      The encrypted secret is sent to the server, along with the expiration
      timestamp.
    </li>
    <li>The server replies with the ID of the secret.</li>
    <li>
      A link is generated by adding the deflated concatenation of the ID,
      encryption key and IV to the hash data of the URL. Hash data is not sent
    ...
    <li>
      The secret value can then be decrypted client side using the information
      stored in the hash data.
    </li>
  </ol>
  <p>
    Want more information? Good news,
    <b-link href="https://github.com/daxAKAhackerman/simple-one-time-secret">this project is open source!
    </b-link>
  </p>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    encryptSecret(secret, key, iv) {
      ...
      const _iv = CryptoJS.enc.Latin1.parse(iv);

      const encrypted = CryptoJS.AES.encrypt(secret, _key, { iv: _iv });
      ...
      const data = encodeURIComponent(
        btoa(
          String.fromCharCode.apply(
            null,
            new Uint16Array(pako.deflate(`${uuid};${key};${iv}`))
          )
        )
      );
